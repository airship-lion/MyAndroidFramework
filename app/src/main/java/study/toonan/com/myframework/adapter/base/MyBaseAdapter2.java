 ...
 */
public abstract class MyBaseAdapter2<T> extends RecyclerAdapter<T> implements RecyclerAdapter.OnLoadMoreListener {
	...
	public MyBaseAdapter2(Context context, List<T> result) {
		this(context, result, DEFAULT_PAGE_SIZE);
	}

	public MyBaseAdapter2(Context context, List<T> result, int pageSize) {
		this(context, result, result.size()>=pageSize);
		mPageSize =pageSize;
	}

	public MyBaseAdapter2(Context context, List<T> result, boolean isLoadMore) {
