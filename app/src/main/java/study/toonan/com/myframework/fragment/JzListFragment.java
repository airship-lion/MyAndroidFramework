 ...
 */
public class JzListFragment extends BaseFragment {
	@Bind(R.id.recyclerview)
	RecyclerView recyclerview;
	@Bind(R.id.swipelayout)
	SwipeRefreshLayout swipelayout;
	protected HttpResult<List<Bean>> result;
	protected List<Bean> mDatas;

	@Override
	protected LoadingPage.LoadResult load() {
		result = new NetManager().getJzListSyn(getJzParam());
		return check(result);
	}

	private Map getJzParam() {
		Map<String, Object> reqMap = new HashMap<>();
		reqMap.put("jsonConditionStr", "{}");
		return reqMap;
	}
