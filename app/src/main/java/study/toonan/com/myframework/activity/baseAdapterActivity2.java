...
public class baseAdapterActivity2 extends AppCompatActivity {
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		mContentPage = new LoadingPage2<List<RentPatrol>>(UIUtils.getContext()) {
			...
			public View createSuccessView() {
				return baseAdapterActivity2.this.createSuccessView();
			}

			@Override
			public Observable<HttpResult<List<RentPatrol>>> Load() {
				return baseAdapterActivity2.this.load();
			}
		};
	...
	private Observable<HttpResult<List<RentPatrol>>> load() {
		return new NetManager().appRentPatrolList(getJzParam());
	}
	...
	private void afterLoadData() {
		currentPage++;
		final CommonAdapter<RentPatrol> adapter = new CommonAdapter<RentPatrol>(baseAdapterActivity2.this,
			...
			protected void convert(ViewHolder holder, RentPatrol o, final int position) {
				...
				holder.getView(R.id.xiaoke_name).setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						Toast.makeText(baseAdapterActivity2.this, mDatas.get(position).getVo().getAqjcfxwt(),
	...
	private void getData(final LoadMoreWrapper mLoadMoreWrapper) {
		load().subscribe(new Subscriber<HttpResult<List<RentPatrol>>>() {
			...
			public void onError(Throwable e) {
				Toast.makeText(baseAdapterActivity2.this, "网络错误", Toast.LENGTH_SHORT).show();
			}
